...
system.time(
  ...
  for(i in 1:length(Cities_US$Urban.Agglomeration)){                      # Pour chaque ville (et pas pour chaque ligne)
    ...
    write.table(mat_Pup, file = paste0("Series/series_Pup_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT'),row.names = F, , sep = ";")
    ...
    write.table(mat_Pdw, file = paste0("Series/series_Pdw_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT'),row.names = F, , sep = ";")
    ...
    write.table(mat_Pur, file = paste0("Series/series_Pur_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT'),row.names = F, , sep = ";")
...
system.time(
  for(i in 1:length(Cities_US$Urban.Agglomeration)){ 
    File_Name_Series <- paste0("Series/series_Pup_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT')     # Le fichier de la premiere période
    ...
    for(n in 1:length(df_series$X1)){ 
      ...
      write.table(mat_var, file = paste0("Angles/Angles_Pup_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT'),row.names = F, , sep = ";")
    }
  }
)
...
system.time(
  for(i in 1:length(Cities_US$Urban.Agglomeration)){ 
    File_Name_Series <- paste0("Series/series_Pdw_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT')     # Le fichier de la premiere période
    File_Name_W_ST4 <- paste0(DIR,"/Bulk/", File_start,Cities_US$Urban.Agglomeration[i],P_level,'.TXT')     # Le fichier de la premiere période
    df_series <- read.table(file = File_Name_Series, sep= ";",header = T) 
    df_series$X1 = as.POSIXct(df_series$X1, tz = 'UTC')
    df_series$X2 = as.POSIXct(df_series$X2, tz = 'UTC')
    ...
    for(n in 1:length(df_series$X1)){ 
      ...
      write.table(mat_var, file = paste0("Angles/Angles_Pdw_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT'),row.names = F, , sep = ";")
    }
  }
)
...
system.time(
  for(i in 1:length(Cities_US$Urban.Agglomeration)){ 
    File_Name_Series <- paste0("Series/series_Pur_winter_", File_start, min_dry_period, '_mdp_', Cities_US$Urban.Agglomeration[i],P_level,'.TXT')     # Le fichier de la premiere période
    File_Name_W_ST4 <- paste0(DIR,"/Bulk/", File_start,Cities_US$Urban.Agglomeration[i],P_level,'.TXT')     # Le fichier de la premiere période
    df_series <- read.table(file = File_Name_Series, sep= ";",header = T) 
    df_series$X1 = as.POSIXct(df_series$X1, tz = 'UTC')
    df_series$X2 = as.POSIXct(df_series$X2, tz = 'UTC')
    ...
    for(n in 1:length(df_series$X1)){ 
      ...
      write.table(mat_var, file = paste0("Angles/Angles_Pur_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT'),row.names = F, , sep = ";")
    }
  }
)
...
system.time(
  for(i in 1:length(Cities_US$Urban.Agglomeration)){ 
    File_Name_dif <- paste0("Angles/Angles_Pup_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT')   
    ...
    write.table(mat_events, file = paste0("Events/Events_Pup_winter_", File_start, min_dry_period, '_mdp_', Cities_US$Urban.Agglomeration[i],P_level,'.TXT'), row.names = F, , sep = ";")
  }
)

system.time(
  for(i in 1:length(Cities_US$Urban.Agglomeration)){ 
    File_Name_dif <- paste0("Angles/Angles_Pdw_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT')   
    df_dif <- read.table(file = File_Name_dif, sep= ";",header = T) 
    File_Name_ST4 <- paste0(DIR,"/Bulk/", File_start,Cities_US$Urban.Agglomeration[i],P_level,'.TXT')     # Le fichier de la premiere période
    df_P <- read.table(file = File_Name_ST4, sep= ";",header = T) 
    df_P$Date <- as.POSIXct(df_P$Date,tz = 'UTC')   # date au format date
    ...
    mat_events <-  na.omit(mat_events)
    colnames(mat_events) <- c('lenght', 'sum', 'max', 'mean')
    write.table(mat_events, file = paste0("Events/Events_Pdw_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT'), row.names = F, , sep = ";")
  }
)
...
system.time(
  for(i in 1:length(Cities_US$Urban.Agglomeration)){ 
    File_Name_dif <- paste0("Angles/Angles_Pur_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT')   
    df_dif <- read.table(file = File_Name_dif, sep= ";",header = T) 
    File_Name_ST4 <- paste0(DIR,"/Bulk/", File_start,Cities_US$Urban.Agglomeration[i],P_level,'.TXT')     # Le fichier de la premiere période
    df_P <- read.table(file = File_Name_ST4, sep= ";",header = T) 
    df_P$Date <- as.POSIXct(df_P$Date,tz = 'UTC')   # date au format date
    ...
    mat_events <-  na.omit(mat_events)
    colnames(mat_events) <- c('lenght', 'sum', 'max', 'mean')
    write.table(mat_events, file = paste0("Events/Events_Pur_winter_", File_start, min_dry_period, '_mdp_',Cities_US$Urban.Agglomeration[i],P_level,'.TXT'), row.names = F, , sep = ";")
  }
)
