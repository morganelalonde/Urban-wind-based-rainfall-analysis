...
Rmax         <-  65 # Rayon autour du point du centre ville
Rmin         <-  30 # Rayon autour du point du centre ville
Rurb         <- sqrt((pi*Rmax^2 - pi*Rmin^2)/4/pi)
...
for (i in 1:length(Names_Cities)) {
  ...
  df_vecU <- as.data.frame(vecU)
  df_vecU$Date <- as.POSIXct(row.names(df_vecU), format="X%Y.%m.%d.%H.%M.%OS")
  df_vecU$Date <- as.Date(df_vecU$Date, "%m/%d/%Y")
  Daily_vecU <- aggregate(vecU ~ Date, df_vecU, FUN='mean')
  Daily_vecU <- Daily_vecU[-1,]
  vecU <- rep(Daily_vecU$vecU, each=24)

  df_vecV <- as.data.frame(vecV)
  df_vecV$Date <- as.POSIXct(row.names(df_vecV), format="X%Y.%m.%d.%H.%M.%OS")
  df_vecV$Date <- as.Date(df_vecV$Date, "%m/%d/%Y")
  Daily_vecV <- aggregate(vecV ~ Date, df_vecV, FUN='mean')
  Daily_vecV <- Daily_vecV[-1,]
  vecV <- rep(Daily_vecV$vecV, each=24)
  ...
  df_Restemp = data.frame(Dates = TIME,
      ...
      df_R = data.frame(Rn = Rmin,
                        Rx= Rmax,
                        Ru= NA)
      df_R$Ru = sqrt(((pi*df_R$Rx^2-pi*df_R$Rn^2)/4)/pi)
      ...
      b_upwindR = b_upwind
      b_upwindR[ras_R <= df_R$Rn | ras_R > df_R$Rx] = NA
      ...
      b_downwindR[ras_R <= df_R$Rn | ras_R > df_R$Rx] = NA
      ...
      b_leftwindR[ras_R <= df_R$Rn | ras_R > df_R$Rx] = NA
      ...
      b_rightwindR[ras_R <= df_R$Rn | ras_R > df_R$Rx] = NA
  ...
  write.table(df_Res_TOT,paste0("Spatial_precip/df_Res_TOT_daily_",Target_city,'_',P_level,".TXT"),row.names = F,sep=';')
